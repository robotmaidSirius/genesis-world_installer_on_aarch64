...
sudo apt install -y libeigen3-dev libjpeg9-dev libpng-dev libpng++-dev
...
sudo apt install -y libqtgui4 libqtwebkit4 libqt4-test libqt4-dev libqt4-opengl-dev python3-pyqt5
...
pushd "${INSTALL_ROOT}" >/dev/null 2>&1
    if [ ! -d ${INSTALL_DIR_OPENCV} ]; then
        wget ${INSTALL_URL_OPENCV} -o opencv-${INSTALL_VER}.zip
        RESULT=$?
        if [ ${RESULT} -ne 0 ]; then
            echo "[ERROR] Failed to download opencv-${INSTALL_VER}.zip"
            exit ${RESULT}
        fi
        unzip opencv-${INSTALL_VER}.zip
        rm opencv-${INSTALL_VER}.zip

        wget ${INSTALL_URL_OPENCV_CONTRIB} -o opencv_contrib-${INSTALL_VER}.zip
        RESULT=$?
        if [ ${RESULT} -ne 0 ]; then
            echo "[ERROR] Failed to download opencv_contrib-${INSTALL_VER}.zip"
            exit ${RESULT}
        fi
        unzip opencv_contrib-${INSTALL_VER}.zip
        rm opencv_contrib-${INSTALL_VER}.zip
    fi
    pushd "${INSTALL_DIR_OPENCV}" >/dev/null 2>&1
    #sed -i 's/include <Eigen\/Core>/include <eigen3\/Eigen\/Core>/g' modules/core/include/opencv2/core/private.hpp
    mkdir -p ${INSTALL_DIR_OPENCV}/build
    pushd "${INSTALL_DIR_OPENCV}/build" >/dev/null 2>&1
        ...
        cmake -D WITH_CUDA=ON \
              ...
              -D OPENCV_EXTRA_MODULES_PATH=../../opencv_contrib-${INSTALL_VER#v}/modules \
              -D WITH_GSTREAMER=ON \
              -D WITH_LIBV4L=ON \
